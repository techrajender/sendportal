...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card" id="completed-campaigns-card">
                <div class="card-header card-header-accent">
                    <div class="card-header-inner">
                ...
                @if($completedCampaigns->hasPages())
                    <div class="card-footer" id="campaigns-pagination-footer">
                        @include('sendportal::layouts.partials.pagination', ['records' => $completedCampaigns])
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="card" id="recent-subscribers-card">
                <div class="card-header card-header-accent">
                    <div class="card-header-inner">
                ...
                @if($recentSubscribers->hasPages())
                    <div class="card-footer" id="subscribers-pagination-footer">
                        @include('sendportal::layouts.partials.pagination', ['records' => $recentSubscribers])
                    </div>
                @endif
            </div>
        </div>
...
@push('js')
    ...
    <script>
        ...
        // Scroll to table on page load if pagination parameters exist
        $(document).ready(function() {
            var urlParams = new URLSearchParams(window.location.search);
            if (urlParams.has('campaigns_page') || urlParams.has('subscribers_page')) {
                // Small delay to ensure page is fully rendered
                setTimeout(function() {
                    if (urlParams.has('campaigns_page')) {
                        var campaignsCard = $('#completed-campaigns-card');
                        if (campaignsCard.length) {
                            $('html, body').animate({
                                scrollTop: campaignsCard.offset().top - 20
                            }, 300);
                        }
                    } else if (urlParams.has('subscribers_page')) {
                        var subscribersCard = $('#recent-subscribers-card');
                        if (subscribersCard.length) {
                            $('html, body').animate({
                                scrollTop: subscribersCard.offset().top - 20
                            }, 300);
                        }
                    }
                }, 100);
        ...
        // Smooth scroll to table when pagination is clicked
        $(document).on('click', '#campaigns-pagination-footer a, #subscribers-pagination-footer a', function(e) {
            var href = $(this).attr('href');
            if (href && href !== '#' && !$(this).hasClass('disabled')) {
                // Store scroll position before navigation
                var targetCard = $(this).closest('.card-footer').closest('.card');
                var cardId = targetCard.attr('id');
                
                // Store in sessionStorage for after page reload
                if (cardId) {
                    sessionStorage.setItem('scrollToCard', cardId);
                }
            }
        });
        
        // Scroll to stored card after page load
        $(window).on('load', function() {
            var scrollToCard = sessionStorage.getItem('scrollToCard');
            if (scrollToCard) {
                setTimeout(function() {
                    var targetCard = $('#' + scrollToCard);
                    if (targetCard.length) {
                        $('html, body').animate({
                            scrollTop: targetCard.offset().top - 20
                        }, 300);
                    }
