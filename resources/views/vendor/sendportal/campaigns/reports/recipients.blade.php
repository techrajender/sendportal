    ...
    <div class="card mb-3">
        <div class="card-header card-header-accent">
            <div class="card-header-inner d-flex justify-content-between align-items-center">
                <span>{{ __('Recipients Report') }}</span>
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse($messages as $message)
                        <tr>
                            <td><a href="{{ route('sendportal.subscribers.show', $message->subscriber_id) }}" class="subscriber-email" data-email="{{ $message->recipient_email }}">{{ $message->recipient_email }}</a></td>
                            <td>{{ $message->subject }}</td>
                            <td>{{ $message->delivered_at ? $message->delivered_at->format('Y-m-d H:i:s') : '-' }}</td>
                            <td>{{ $message->opened_at ? $message->opened_at->format('Y-m-d H:i:s') : '-' }}</td>
                            <td>{{ $message->clicked_at ? $message->clicked_at->format('Y-m-d H:i:s') : '-' }}</td>
                            <td>{{ $message->bounced_at ? $message->bounced_at->format('Y-m-d H:i:s') : '-' }}</td>
                            <td>{{ $message->complained_at ? $message->complained_at->format('Y-m-d H:i:s') : '-' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="100%">
