...
@section('content')
    ...
    @component('sendportal::layouts.partials.card')
        ...
        @slot('cardBody')
            <form action="{{ route('sendportal.tags.update', $tag->id) }}" method="POST" class="form-horizontal" id="tag-form">
                @csrf
                @method('PUT')
                            ...
                            <ul class="nav nav-tabs mb-2" id="subscriberTabs" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" id="selected-tab" data-toggle="tab" href="#selected-subscribers" role="tab" aria-controls="selected-subscribers" aria-selected="false">
                                        {{ __('Selected') }} (<span id="selected-count">{{ count($selectedSubscriberIds ?? []) }}</span>)
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" id="not-selected-tab" data-toggle="tab" href="#not-selected-subscribers" role="tab" aria-controls="not-selected-subscribers" aria-selected="false">
                                        {{ __('Not Selected') }} (<span id="not-selected-count">{{ count($subscribers ?? []) - count($selectedSubscriberIds ?? []) }}</span>)
                                    </a>
                                </li>
                            </ul>
                                            ...
                                            <div class="checkbox subscriber-item" 
                                                ...
                                                <label>
                                                    <input type="checkbox" 
                                                           ...
                                                           {{ in_array($subscriber->id, ($selectedSubscriberIds ?? [])) ? 'checked' : '' }}>
